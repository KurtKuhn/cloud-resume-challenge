...
resource "aws_cloudfront_distribution" "resume_distribution" {
  ...
  origin {
    ...
    origin_id   = aws_s3_bucket_website_configuration.resume_config.website_endpoint
  ...
  default_cache_behavior {
    allowed_methods  = ["GET", "HEAD"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = aws_s3_bucket_website_configuration.resume_config.website_endpoint
    compress         = true
